...
var mSelecter_1 = __webpack_require__(1);
__webpack_require__(4);
var mySelecter = new mSelecter_1.default({
    el: '#mSelecterBtn',
    transition: true,
    relation: true,
    data: [
...
var mySelecter1 = new mSelecter_1.default({
    el: '#mSelecterBtn1',
    transition: true,
    relation: false,
    data: [
...
var mSelecter = /** @class */ (function () {
    function mSelecter(option) {
    ...
    mSelecter.prototype.init = function () {
        this.initOption();
        this.initLayer();
    ...
    mSelecter.prototype.initOption = function () {
        this.option = Object.assign(this.defaultOption, this.option || {});
        // 处理传进来的默认选项, 设置当前选项
        this.currentItem = this.getSelected();
    };
    // 处理传进来的默认选项
    mSelecter.prototype.getSelected = function () {
        return this.option.relation ? this.getRelationSelected() : this.getNotRelationSelected();
    };
    mSelecter.prototype.getRelationSelected = function () {
        var selected = this.option.defaultSelect, lastChild = null;
        // 第一级的处理特殊一点
    ...
    mSelecter.prototype.getNotRelationSelected = function () {
        var selected = this.option.defaultSelect, data = this.option.data;
    ...
    mSelecter.prototype.initLayer = function () {
        // 获取scrollList
        var listHtml = '';
    ...
    mSelecter.prototype.updateLayer = function (fromIndex) {
        var listHtml = '', originScrollListHtml = this.scrollListHtml.join('');
        this.updateScrollListHtml(fromIndex);
        listHtml = this.scrollListHtml.join('');
        // 相同的话 不操作dom
        if (originScrollListHtml === listHtml)
            return;
        this.wrap.find('.mSelecterList').html(listHtml);
    };
    // 更新scrollList的html
    mSelecter.prototype.updateScrollListHtml = function (fromIndex) {
        var from = typeof fromIndex == 'undefined' ? 0 : parseInt(fromIndex) + 1, 
        // 初始化页面上显示的数据 (整理成二维数组， 每一列的数据是一个数组项)
        initData = this.option.relation ? this.getInitData() : this.option.data, listHtml = '';
    ...
    mSelecter.prototype.getInitData = function () {
        var result = [], tempResult = [], data = this.option.data, selected = this.currentItem || [];
    ...
    mSelecter.prototype.registerMSelecter = function () {
        var _this = this;
        this.wrap.hide();
    ...
    mSelecter.prototype.initUi = function () {
    ...
    mSelecter.prototype.initSelect = function (result) {
        var _this = this;
        this.scrollList.each(function (index, item) {
            ...
            _this.scrollList
                ...
                .addClass('active');
            // 滚动到选中位置
            _this.setTranslateY(-result[index] * _this.itemHeight, _this.scrollList.eq(index));
            // 存储当前滚动列的滚动高度，方便后面滚动计算
            _this.currentTranslateY.push(-result[index] * _this.itemHeight);
        });
        // 存储当前选中项，方便后面计算
        this.currentItem = result;
    };
    mSelecter.prototype.initEvents = function () {
        var _this = this;
        // 去掉初始化事件，重新绑定
        this.el.off('click');
        this.el.on('click', function () {
        ...
        this.wrap.find('.mSelecterList').on('touchstart', '.mSelecterItemList', function (event) {
            ...
            _this.pointY = event.touches[0].pageY;
            _this.currentScrollList = _this.wrap.find('.mSelecterItemList').eq(index);
            _this.currentScrollListIndex = index;
            _this.beforeScrollCurrentItemIndex = _this.currentItem[_this.currentScrollListIndex];
            _this.option.transition && _this.initTransition();
            return false;
        });
        this.wrap.find('.mSelecterList').on('touchmove', '.mSelecterItemList', function (event) {
            var translateY = _this.getTranslateY(event.touches[0].pageY);
            _this.touchFeedback(translateY, 'touchmove');
            return false;
        });
        this.wrap.find('.mSelecterList').on('touchend', '.mSelecterItemList', function (event) {
            _this.touchFeedback(_this.fixedTranslateY(), 'touchend');
            return false;
        });
        this.confirm.on('click', function () {
            var result = _this.getResult();
            typeof _this.option.callback == 'function' && _this.option.callback(result);
            _this.wrap.hide();
        });
        ...
        $(window).on('resize', function () {
            clearTimeout(_this.resizedFinished);
            _this.resizedFinished = setTimeout(function () {
                _this.initUi();
            }, 300);
        });
    };
    // 获取每次触摸的位移
    mSelecter.prototype.getTranslateY = function (currentPointY) {
    ...
    mSelecter.prototype.touchFeedback = function (translateY, eventType) {
    ...
    // 更新选中数组
    mSelecter.prototype.updateCurrentItem = function (index) {
        var currentItem = this.currentItem, lastChild = this.option.data[currentItem[0]];
        // 获取当前最后一列值
    ...
    // 更新当前列的active
    mSelecter.prototype.updateTouchEndUi = function () {
        var mSelecterItemList = this.wrap.find('.mSelecterItemList'), currentItemList = null;
    ...
    mSelecter.prototype.updateTouchMoveUi = function () {
    ...
    // 给回调函数传参
    mSelecter.prototype.getResult = function () {
        return this.option.relation ? this.getRelationResult() : this.getNotRelationResult();
    };
    mSelecter.prototype.getRelationResult = function () {
        var result = [], currentItem = this.currentItem, lastChild = this.option.data[currentItem[0]];
    ...
    mSelecter.prototype.getNotRelationResult = function () {
        var result = [], currentItem = this.currentItem;
    ...
    // 获取数组中，属性值等于给定值的下标
    mSelecter.prototype.getArrayAttrIndex = function (obj) {
        var attr = obj.attr, value = obj.value, array = obj.array;
    ...
    mSelecter.prototype.initTransition = function () {
